...
class SlackBrowse:
    ...
    def channels(self, info, autoload_path, ch_id, ch_name):
        """
        API Document:
        ...
        line = 0
        channel_list = SlackORM().channels(autoload_path)
    ...
    def choice_ch(self, info, ch, autoload_path):
    ...
    @exception
    def history(self, info, count, autoload_path):
        """
        API Document
        ...
        if res.json().get("ok"):
            if os.path.isfile(autoload_path + "/SlackInfoData"):
                user_list = SlackORM().members(autoload_path)
...
def show_channels(info, autoload_path, ch_id, ch_name):
    """
    Show SlackChannels
    ...
    if os.path.isfile(autoload_path + "/SlackInfoData"):
        if isinstance(info, dict):
            (line_num, ch_name) = SlackBrowse().channels(info, autoload_path, ch_id, ch_name)
...
def choice_channel(info, ch, autoload_path):
    """
    ...
    if os.path.isfile(autoload_path + "/SlackInfoData"):
        if isinstance(info, dict):
            return SlackBrowse().choice_ch(info, ch, autoload_path)
        else:
            print (info)
    else:
        print ('Plz run the "SlackMode"')
...
def show_history(info, count, autoload_path):
    """ Show Channel History """
    if isinstance(info, dict):
        return SlackBrowse().history(info, count, autoload_path)
    else:
        print (info)
...
class SlackORM:
    ...
    def mode_on(self, autoload_path, info):
        ...
        slack_info_file = autoload_path + "/SlackInfoData"

        if isinstance(info, dict):
            if os.path.isfile(slack_info_file):
                os.remove(slack_info_file)
            else:
                pass

            member_info = _get_member_list(self.__users_list_api, info)
            time.sleep(1)
            channel_info = _get_channel_list(self.__channels_list_api, info)

            slack_mode_info = {"ml": member_info, "cl": channel_info}

            with open(slack_info_file, "wb") as f:
                pickle.dump(slack_mode_info, f)
    ...
    def members(self, autoload_path):
        """ load members """
        with open(autoload_path + "/SlackInfoData") as f:
            slack_mode_info = pickle.load(f)

        return slack_mode_info.get("ml")
